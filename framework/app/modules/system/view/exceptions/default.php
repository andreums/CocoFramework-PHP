...
<div id="kohana_error">
    <h1><span class="type"><?php //echo $exception->getType(); ?> [ <?php echo $exception->getCode(); ?> ]:</span> <span class="message"><?php echo $exception->getMessage(); ?></span></h1>
    <div id="<?php echo $error_id ?>" class="content">
        <p><span class="file"><?php echo $exception->getFile(); ?> [ <?php echo $exception->getLine(); ?> ]</span></p>
        <?php echo FW_Debug::source($exception->getFile(),$exception->getLine()); ?>        
        <ol class="trace">
        <?php foreach (FW_Debug::trace($exception->getTrace()) as $i => $step):?>
            <li>
                <p>
                    <span class="file">
                        ...
                        <?php else: ?>
                            {<?php echo _('PHP internal call') ?>}
                        <?php endif ?>
                    </span>
                    &raquo;
                    <?php echo $step['function'] ?>(<?php if ($step['args']): $args_id = $error_id.'args'.$i; ?><a href="#<?php echo $args_id ?>" onclick="return koggle('<?php echo $args_id ?>')"><?php echo _('arguments') ?></a><?php endif ?>)
                </p>
                ...
                <div id="<?php echo $args_id ?>" class="collapsed">
                    ...
                    <?php foreach ($step['args'] as $name => $arg): ?>
                        <tr>
                            ...
                            <td><pre><?php echo FW_Debug::dump($arg) ?></pre></td>
                        </tr>
                    <?php endforeach ?>
                    </table>
                </div>
    ...
    <h2><a href="#<?php echo $env_id = $error_id.'environment' ?>" onclick="return koggle('<?php echo $env_id ?>')"><?php echo _('Environment') ?></a></h2>
    <div id="<?php echo $env_id ?>" class="content collapsed">
        <?php $included = get_included_files() ?>
        <h3><a href="#<?php echo $env_id = $error_id.'environment_included' ?>" onclick="return koggle('<?php echo $env_id ?>')"><?php echo _('Included files') ?></a> (<?php echo count($included) ?>)</h3>
        ...
        <h3><a href="#<?php echo $env_id = $error_id.'environment_loaded' ?>" onclick="return koggle('<?php echo $env_id ?>')"><?php echo _('Loaded extensions') ?></a> (<?php echo count($included) ?>)</h3>
        <div id="<?php echo $env_id ?>" class="collapsed">
            <table cellspacing="0">
                <?php foreach ($included as $file): ?>
                <tr>
        ...
        <div id="<?php echo $env_id ?>" class="collapsed">
            <table cellspacing="0">
                ...
                <tr>
                    ...
                    <td><pre><?php echo FW_Debug::dump($value); ?></pre></td>
                </tr>
                <?php endforeach ?>
            </table>
        </div>
