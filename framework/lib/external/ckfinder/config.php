...
function CheckAuthentication()
{
    ...
    print "Paso por aquí!";
    $result = FW_Authentication::checkUserAuthentication();
    if (!$result) {
        return false;
    }    
    $user = FW_Authentication::getUser();
    $user = array("username"=>$user->username,"roles"=>$user->getRoles(),"status"=>$user->status);
    FW_Session::set("user",$user,"ckfinder");
    
    // comprovar els rols de l'usuari
    $roles = $user["roles"];
    for ($i=0;$i<count($roles);$i++) {
        if (in_array($roles[$i],array("root","administrador","profesional","banco","inmobiliaria","promotor","editor"))) {
            chdir($old);
            return true;            
        }
    }
...
if (isset($_SESSION["FRAMEWORK_SESSION"]["ckfinder"]["user"])) {
    $user = $_SESSION["FRAMEWORK_SESSION"]["ckfinder"]["user"];
}
else {
...
$hash            = md5(sha1($username));
$baseUrl      = "/uploads/users/{$hash}/";

if ( (in_array("root",$roles)) || (in_array("administrator",$roles)) || (in_array("editor",$roles))  ) {
    $baseUrl = "/uploads/";
}
...
$baseUrl = "http://www.vendoyodev.es{$baseUrl}";

$config['UserFilesPath'] = "http://www.vendoyodev.es/";
...
*/
$config['HideFolders'] = Array(".svn", "CVS",".git","framework","app");
